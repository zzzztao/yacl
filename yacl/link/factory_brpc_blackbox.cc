...
void FactoryBrpcBlackBox::GetPartyNodeInfoFromEnv(
  ...
  YACL_ENFORCE(iter != party_info.end(), "cannot find self_party:{} node info.",
               self_party_id);
...
std::shared_ptr<Context> FactoryBrpcBlackBox::CreateContext(
  ...
  options = ChannelBrpcBlackBox::MakeOptions(
  ...
  auto msg_loop = std::make_unique<ReceiverLoopBlackBox>();
  ...
  for (size_t rank = 0; rank < world_size; rank++) {
    ...
    auto channel = std::make_shared<ChannelBrpcBlackBox>(
