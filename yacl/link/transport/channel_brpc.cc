...
class ReceiverServiceImpl : public ic_pb::ReceiverService {
 public:
  explicit ReceiverServiceImpl(
      std::map<size_t, std::shared_ptr<ChannelBrpcBase>> listener)
 ...
 protected:
  std::map<size_t, std::shared_ptr<ChannelBrpcBase>> listeners_;
};
...
class SendChunkedDone : public google::protobuf::Closure {
 public:
  SendChunkedDone(std::string chunk_info, SendChunkedWindow& w)
      : chunk_info_(std::move(chunk_info)), wait_(w) {}

  void Run() override {
    std::unique_ptr<SendChunkedDone> self_guard(this);
    if (cntl_.Failed()) {
      SPDLOG_ERROR("{} rpc failed: {}, message={}", chunk_info_,
                   cntl_.ErrorCode(), cntl_.ErrorText());
      exit(1);
    } else if (response_.header().error_code() != ic::ErrorCode::OK) {
      SPDLOG_ERROR("{} response failed, message={}", chunk_info_,
 ...
 private:
  std::string chunk_info_;
  SendChunkedWindow& wait_;
};

}  // namespace

void ChannelBrpc::PushRequest(org::interconnection::link::PushRequest& request,
                              uint32_t timeout) {
...
void ChannelBrpc::AsyncSendChunked(ic_pb::PushRequest& request,
                                   SendChunkedWindow& window,
                                   std::string chunk_info) {
  auto* done = new SendChunkedDone(std::move(chunk_info), window);
